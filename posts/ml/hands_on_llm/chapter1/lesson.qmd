...
**Negative sampling** is used to speed up the next-word prediction process. Instead of predicting the next token (a computationally expensive neural network), we reframe the task as "given two words, what is the probability that they are neighbours?" (a much faster logisitc regression problem.)
...
To avoid this, we introduce negative exmaples by taking random combinations of words in the vocabulary that aren't neighbours.
