...
Variant can store up to **16 MB per row**. 
...
Time travel SQL query within a retention period. The AT keyword allows us to time travel. 
...
Alternatively we can use BEFORE, where query ID is the ID where we messed things up :

SELECT * FROM table BEFORE (STATEMENT>= query_id)

Go to Activity -> Query History in the Snowsight UI to get the right query ID. 

It is best practice to recover to an intermediate backup table first to confirm the recovered version is correct. Use 
...
## Retention Period 
Retention period is the number of days for which historical data is preserved. Determines how far back we can time travel. 

Configurable for table, schema, database or account. 

The default DATA_RETENTION_TIME_IN_DAYS=1
If we want to disable time travel, we can set this to 0. 

We can set this when we create a table or alter is for an existing table
ALTER TABLE table_name (
SET DATA_RETENTION_TIME_IN_DAYS=2)

We can set a minimum value at the account level
ALTER ACCOUNT SET 
MIN_ DATA_RETENTION_TIME_IN_DAYS=3

The account type determines the maximum retention period. 
Standard - 1
Enterprise or higher - 90


## Fail Safe

This is for recovery beyond time travel.
This is not configurable and set to 7 days beyond time travel period for permanent tables. 

This is for disaster recovery. We as users cannot access this data. We have to contact Snowflake support to restore it for us. 


## Storage costs
Time travel and fail safe contribute to storage costs. 
You only pay for what is modified. 
You can see the breakdown in the Admin -> Usage tab. 

You can see how much storage is being used with

SELECT * FROM SNOWFLAKE_ACCOUNT_USAGE.TABLE_STORAGE_METRICS


## Table types

Permanent - CREATE TABLE. 0-90 days time travel + fail safe. Persist until dropped. 

Transient - CREATE TRANSIENT TABLE. 0 or 1 day time travel. No fail safe. Persist until dropped.  For large tables that do not need to be protected. 

Temporary - CREATE TEMPORARY TABLE. 0 or 1 day time travel. No fail safe. Only in session - deleted after the worksheet is closed. Non-permanent data just for this session. 
...
Temporary table names do not clash with permanent or transient tables. Temporary table takes precedence in the session and hides the others with the same name. 
...
Makes it easy to copy an existing object. 
...
*the exceptions are pipes which can only be cloned if referencing an external stage, and stages which cannot be cloned for named internal stages. 
When we clone a database or schema, all of its child objects are also cloned. 

Copy diagram at 3:20
...
When modifying the “copy” Snowflake only stored the deltas, not the entire database again. The “original” and “copy” can be modified independently without affecting the other. 

Can be used to create backups for dev work. 

## Privileges. 
The privileges of the CHILD objects are inherited from the clone source, but the cloned object itself (ie the cloned database or schema) are NOT inherited, they need to be specified separately by the administrator. 
