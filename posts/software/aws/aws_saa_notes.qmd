...
Disaster recovery whitepaper [here](https://aws.amazon.com/disaster-recovery/)
...
1. **Backup and restore**. Periodically save to S3 or save snapshots for EBS, Redshift and RDS. 
2. **Pilot light**. A small version of the app is always running in the cloud; this is the critical core AKA the pilot light. This is faster to restore as critical systems are already running. E.g. we might just have the RDS database running but not any EC2 instances. 
3. **Warm standby**. The *full system* is up and running but at minimum size, ready to be scaled up in the event of a disaster. 
4. **Hot site / multi-site approach**. Full production scale is running on AWS in parallel to the on premises version. If fully in cloud, this would be replicating across multiple regions. 
...
![DMS Continuous Replication](dms_continuous_replication.png)
...
![DMS Multi-AZ Deployment](dms_multi_az.png)


### 25.3.5. RDS and Aurora MySQL Migrations

This is a specific example/gotcha when using DMS. 
If we want to migrate RDS MySQL to Aurora MySQL, we have several options:

1. Take RDS snapshots and restore them as Aurora DB
2. Create an Aurora read replica from RDS, and eventually promote it as its own cluster once fully copied. 
3. Use Percona XtraBackup to create a backup dump in S3 and create an Aurora database from this
4. Use the mysqldump utility to migrate MySQL to Aurora. 
5. Use DMS if both databases are running to do continuous replication. 
...
**A fully managed service to centrally manage and automate backups across AWS services**: EC2 / EBS, S3, RDS, Aurora, DynamoDB, DocumentDB / Neptune, EFS, Storage Gateway. 
It backs up everything to an internal S3 bucket specific to AWS Backup. 

It supports cross-region backups and cross-account backups. This is helpful for DR. 

It supports PITR for supported services. Backups can be on-demand or scheduled. 
You create tag-based backup policies called Backup Plans, specifying the frequency, window, transition lifecycle to cold storage, retention period, etc.
...
![Event Processing with SNS, SQS, Lambda](event_architecture_sqs.png)
...
![Event Architecture Fan Out](event_architecture_fan_out.png)

S3 Event Notifications. Use this to react to changes in an S3 bucket, e.g. a user uploading a video. 

![Event Architecture S3 Events](event_architecture_s3_events.png)
...
![Event Architecture EventBridge](event_architecture_eventbridge.png)
...
![Caching Architecture](caching_architecture.png)
...
![Blocking an IP Address Architecture - ALB, CF, WAF](blocking_an_ip_architecture.png)
...
![Highly Available EC2 Instance](highly_available_ec2.png)
...
![Highly Available EC2 Instance - ASG](highly_available_ec2_asg.png)
...
![Highly Available EC2 Instance - ASG + EBS](highly_available_ec2_ebs.png)
...
![CloudFormation + Infrastructure Composer](cloudformation.png)
...
![CloudFormation Service Role](cloudformation_service_role.png)
...
There is a choice of IP addresses to send from: shared, dedicated, customer-owned. 
It supports DomainKeys Identified Mail (DKIM) and Sender Policy Framework (SPF) protocols. 
...
No SSH access, bastion hosts, or SSH keys needed, and no port 22 needed, which is better for security.
...
1. SSH via port 22. Requires ssh keys to be added on instance creation.
2. EC2 Instance Connect. Requires ssh keys and port 22 inbound rules allowed.
3. Using SSM Session Manager

![SSM Session Manager](ssm_session_manager.png)
...
**AWS Outposts are “server racks” that offers the same AWS infrastructure**, services, APIs, tools etc.
This means the on-premises infra is the same as the cloud infra, making it easier to manage both.
This can also aid in migrating to the cloud.

AWS will come to your physical location and set up **Outpost Racks** with services pre-loaded.

![AWS Outpost](outpost.png)
...
Think of it like Elastic Beanstalk for web / mobile apps. 
It is an abstraction over many AWS services in the background that handle storage, auth, processing, etc, so you just manage it all through one portal.

![AWS Amplify](aws_amplify.png)
...
It supports EC2 instances, EC2 Auto Scaling Groups, RDS instances.
Schedules are managed in a DynamoDB table and use tags on resources to identify what to start/stop, with Lambda performing the stop/start actions.
...
The weel-architected framework is documented on the AWS website [here](https://docs.aws.amazon.com/wellarchitected/latest/framework/welcome.html
).
...
These are **synergies not tradeoffs**. The pillars are detailed in separate whitepapers [here](https://aws.amazon.com/blogs/aws/aws-well-architected-framework-updated-white-papers-tools-and-best-practices/).
...
The **Well-Architected Tool** is available in the AWS console. You define your workload and answer questions about it, then select the lens through which you want to evaluate it, e.g. the well-architected framework.
...
Trusted Advisor gives a **high-level assessment of your AWS account**.
For example, it can flag if you have any public EBS or RDS snapshots, or whether your root account is used instead of IAM roles.
...
- Architecture guide with examples and explanations of many different architectures [here](https://aws.amazon.com/architecture/)
- Solutions library with examples and CloudFormation code for different architectures [here](https://aws.amazon.com/solutions/)


# 29. Exam Preparation

- Refer to the exam syllabus [here](https://aws.amazon.com/certification/certified-solutions-architect-associate/)
- Proceed by elimination. There are very few trick questions. If a solution seems feasible but highly complicated, it’s probably wrong.
- Skim-read the white papers.
- Read the FAQs for a service, e.g. [here](https://aws.amazon.com/vpc/faqs/)
