...
$$
MSE(\hat{\theta}) = Bias(\hat{\theta}) ^2 + Var(\theta)
$$
...
$$
Efficiency(\hat{\theta}) = CRLB / Var(\hat{\theta})
$$
...
$$
Relative Efficiency = Var(\hat{\theta}_{MOM}) / Var(\hat{\theta}_{MLE})
$$
...
Mathematically
$$
P(| \hat{\theta} - \theta | < \epsilon) -> 1  as n -> \inf
...
Mathematically:
$$
P(\theta - 2\sigma(\hat{\theta}) <= \hat{\theta} <= \theta  2\sigma(\hat{\theta})) = 0.95
$$
...
$$
P(-\hat{\theta} - 2\sigma(\hat{\theta}) <= \theta <= -\hat{\theta} + 2\sigma(\hat{\theta})) = 0.95
$$
...
In probability, we can make statements like "there is 95% probability that $\hat{\theta}$ is in $\theta +- 2 \sigma(\theta)$".

In statistics, thanks to our pivot (algebraic manipulation of the inequality), we make statements like "we are 95% confidence that $\theta$ is in the interval \hat{\theta} +- 2 * standard error(\hat{\theta})$".
