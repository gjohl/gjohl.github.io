...
Time spreads are always quoted chronologically. Contango markets are characterised by negative time spreads. 
...
![Forward curve in backwardation](backwardation.png)


## Contango
Contango market structures are characterised by negative time spreads; each contract period is followed by another where the price is higher. It is a structure that is symptomatic of a well-supplied/oversupplied market, with low prices close to delivery gradually increasing into the future. Contango markets are generally bearish, with weak fundamentals because of the oversupply.

![Forward curve in contango](contango.png)


## Trading time spreads

Due to correlation, the individual contracts will move together to some extent, but one will be impacted more than the other, affecting the spread.

The news will always impact both legs of a spread, but the impact of fundamental supply and demand changes will be felt most by the one closest to the event.

Time spreads are very commonly used instead of flat price by fundamental traders. Suppose a trader can't predict the overall direction of a commodity because speculation is having a greater impact than fundamentals. In that case, they can trade the front spread, as it should reflect the fundamentals with less impact from speculation.